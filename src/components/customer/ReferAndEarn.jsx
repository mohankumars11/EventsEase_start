    ...
    <section className="bg-gradient-to-br from-saffron-500 to-amber-600 rounded-3xl p-6 sm:p-7 text-white relative overflow-hidden">
      ...
      <div className="relative flex flex-col sm:flex-row sm:items-center gap-5">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <div className="hidden sm:block bg-white/15 rounded-xl px-3 py-2 text-xs font-mono truncate max-w-[180px]">
            {link.replace(/^https?:\/\//, '')}
          </div>
          <button
            onClick={copyLink}
          ...
          <button
            onClick={shareLink}
            className="sm:hidden flex items-center justify-center bg-white/20 hover:bg-white/30 p-2.5 rounded-xl transition-colors"
            aria-label="Share"
          >
            <Share2 size={15} />
          </button>
