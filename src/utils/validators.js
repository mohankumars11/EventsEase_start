...
export function getAddressErrors({ name, phone, line, city, pincode }) {
  ...
  // The city list is read, never typed. This string used to spell out
  // "Bengaluru & Mysore" by hand, so opening a third city meant finding a
  // validator message to keep the app honest.
  else if (!isLiveCity(city))       errors.city = `We don't deliver here yet — currently live in ${LIVE_CITIES.map(c => c.name).join(' & ')} only`
