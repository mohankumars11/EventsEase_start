...
export default async function handler(req, res) {
  ...
  const { data: created, error: reqErr } = await db.rpc('create_booking_request', {
    ...
    /* The radius the customer asked for, capped at what dispatch will
       ...
       This briefly read `Math.min(radiusKm, MAX_RADIUS_KM, 25)`. Migration
       058 had created the column with CHECK (radius_km BETWEEN 1 AND 25),
       written when 25 km was the widest anyone could ask for; 086 then
       took the radius out of the filter entirely and MAX_RADIUS_KM moved
       to 60. The CHECK did not move with it, so every booking that asked
       for 60 was rejected by the database and instant booking returned a
       500 to everyone for about an hour.

       Migration 088 raised the CHECK to 100 -- matching
       vendors.service_radius_km from 057 -- so the clamp has done its job
       and the column can hold the truth again.

       It is not only bookkeeping: standing lines re-match against
       `r.radius_km` (069), so a stored 25 quietly narrowed the re-match
       for every parked line to less than half the city. */
