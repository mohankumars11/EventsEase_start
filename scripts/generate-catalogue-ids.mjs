...
for (const menu of M.ALL_MENUS) {
  ...
  for (const line of M.menuLines(menu)) {
    ...
    const lineKey = `line|${menu.id}|${norm(text)}`
    take('MLN', lineKey, `line · ${menu.id} · ${text}`)

    /* A line naming more than one dish needs a row per dish, and each of
       those rows needs an id. Minted from the SAME resolver the seed
       uses — two copies of this decision would mint an id for a dish the
       seed never writes, or leave the seed pointing at an id that does
       not exist. */
    const r = RESOLVE.resolveMenuLine(text)
    for (const opt of r.options) {
      take('MLO', `option|${menu.id}|${norm(text)}|${norm(opt.name)}`,
        `option · ${menu.id} · ${text} · ${opt.name}`)
    }
  }
}
